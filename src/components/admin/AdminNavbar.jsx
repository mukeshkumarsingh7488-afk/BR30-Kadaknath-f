...
const AdminNavbar = ({ theme, onToggleTheme, onMenuClick }) => {
  const user = getStoredUser();

  return (
    <header className="admin-navbar">
      <div className="admin-navbar-left">
        <button type="button" className="admin-menu-button" onClick={onMenuClick} aria-label="Open menu">
          <Menu size={21} />
        </button>
      ...
      <style>{`
        ...
        .admin-navbar-user{display:flex;align-items:center;gap:9px;margin-left:4px;padding-left:12px;border-left:1px solid var(--admin-border)}
        ...
        @media (max-width:900px){.admin-navbar{padding:0 16px}.admin-menu-button{display:grid;place-items:center}}
        @media (max-width:600px){.admin-navbar{height:68px}.admin-page-context{display:none}.admin-theme-button span,.admin-navbar-user-info,.admin-navbar-user>svg{display:none}.admin-theme-button{width:40px;padding:0}.admin-navbar-user{border-left:0;padding-left:0}}
      `}</style>
    </header>
  );
};
