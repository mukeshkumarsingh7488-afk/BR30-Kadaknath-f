...
const CustomersStaff = () => {
  ...
  const [stats, setStats] = useState({ total: 0, customers: 0, staff: 0, admin: 0, fm: 0, security: 0, blocked: 0 });
  ...
  const handleRoleChange = async (user, newRole) => {
    ...
    if (user.role === "admin") {
      await showError("Role Change Not Allowed", "Admin account role cannot be changed.");
      return;
    }
    ...
    const getRoleLabel = (role) => {
      ...
      if (role === "fm") return "Farm Manager";
  ...
  return (
    <div className="customers-staff-page">
      <style>{`
        ...
        .role-select:disabled{opacity:.65;cursor:not-allowed;background:var(--admin-surface-2)}
      ...
      <div className="customers-toolbar">
        ...
        <div className="customers-filter">
          <select value={roleFilter} onChange={(event) => setRoleFilter(event.target.value)}>
            ...
            <option value="fm">Farm Manager</option>
                  ...
                  <tr key={user.id}>
                    ...
                    <td>
                      <span className={`role-badge ${user.role}`}>
                        {user.role === "admin" ? <ShieldCheck size={12} /> : user.role === "staff" ? <ShieldCheck size={12} /> : user.role === "fm" ? <ShieldCheck size={12} /> : user.role === "security" ? <ShieldCheck size={12} /> : <UserCheck size={12} />}
                        {user.role === "admin" ? "Admin" : user.role === "staff" ? "Staff" : user.role === "fm" ? "Farm Manager" : user.role === "security" ? "Security" : "Customer"}
                      </span>
                    </td>
                      ...
                      <div className="actions-cell">
                        ...
                        {user.role === "admin" ? (
                          <div className="role-select-wrap">
                            <select className="role-select" value="admin" disabled title="Admin role cannot be changed">
                              <option value="admin">Admin</option>
                            </select>
                          </div>
                        ) : (
